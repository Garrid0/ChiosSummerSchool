...
class LCA():
    ...
    def __init__(self, metadata,theta):
        ...
        if self.n_alternatives == 2:
            self.v0 = theta['v0']#params['v0']
            self.v1 = theta['v1']#params['v1']#
        
        if self.n_alternatives == 3:
            self.v0 = theta['v0']#params['v0']#theta['v0']
            self.v1 = theta['v1'] #params['v1']#theta['v1']
            self.v2 = theta['v2'] #params['v2']#theta['v2']
            ...
            if self.use_tqdm:
                ...
                for n in tqdm(range(self.n_sims)):
                    ...
                    for t in range(self.n_max_steps):
                        ...
                        x0[t+1] = x0[t] + self.offset + (self.v0 + self.leak*x0[t] + self.inh*x1[t])*self.dt_tau + noise[t,0]*self.sqrt_dt_tau
                        x1[t+1] = x1[t] + self.offset + (self.v1 + self.leak*x1[t] + self.inh*x0[t])*self.dt_tau + noise[t,1]*self.sqrt_dt_tau
            ...
            if self.use_tqdm:
                ...
                for n in tqdm(range(self.n_sims)):
                    ...
                    for t in range(self.n_max_steps):
                        ...
                        x0[t+1] = x0[t] + self.offset + (self.v0 + self.leak*x0[t] + self.inh*(x1[t] + x2[t]))*self.dt_tau + noise[t,0]*self.sqrt_dt_tau
                        x1[t+1] = x1[t] + self.offset + (self.v1 + self.leak*x1[t] + self.inh*(x0[t] + x2[t]))*self.dt_tau + noise[t,1]*self.sqrt_dt_tau
                        x2[t+1] = x2[t] + self.offset + (self.v2 + self.leak*x2[t] + self.inh*(x0[t] + x1[t]))*self.dt_tau + noise[t,2]*self.sqrt_dt_tau
