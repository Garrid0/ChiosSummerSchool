        ...
        for p in tqdm(range(self.n_par)):
            ...
            for t in range(self.n_trials):
                ...
                if n_options == 2:
                    ...
                    if self.drift_rate_model == "q_cv":
                        v0 = self.v_mod * Q_values[t,best_option-1]/np.sqrt(Q_values_var[t,best_option-1])
                        v1 = self.v_mod * Q_values[t,worst_option-1]/np.sqrt(Q_values_var[t,worst_option-1])

                    if self.drift_rate_model == "q_cv_unc_bonus":
                        v0 = self.alpha * Q_values[t,best_option-1]/np.sqrt(Q_values_var[t,best_option-1]) + self.beta*np.sqrt(Q_values_var[t,best_option-1])
                        v1 = self.alpha * Q_values[t,worst_option-1]/np.sqrt(Q_values_var[t,worst_option-1]) + self.beta*np.sqrt(Q_values_var[t,worst_option-1])
                    
                    if self.drift_rate_model == "q_mean_unc_bonus":
                        v0 = self.alpha * Q_values[t,best_option-1] + self.beta*np.sqrt(Q_values_var[t,best_option-1])
                        v1 = self.alpha * Q_values[t,worst_option-1] + self.beta*np.sqrt(Q_values_var[t,worst_option-1])
                    ...
                    if self.eam_model == "race":
                        theta_eam = {'v0': v0, 'v1': v1, 'a': self.a,'t': self.ndt}
                ...
                if n_options == 3:
                    ...
                    if self.drift_rate_model == "q_cv":
                        ...
                        v0 = self.v_mod * Q_values[t,best_option-1]/np.sqrt(Q_values_var[t,best_option-1])
                        v1 = self.v_mod * Q_values[t,second_best_option-1]/np.sqrt(Q_values_var[t,second_best_option-1])
                        v2 = self.v_mod * Q_values[t,worst_option-1]/np.sqrt(Q_values_var[t,worst_option-1])

                    if self.drift_rate_model == "q_cv_unc_bonus":
                        v0 = self.alpha * Q_values[t,best_option-1]/np.sqrt(Q_values_var[t,best_option-1]) + self.beta*np.sqrt(Q_values_var[t,best_option-1])
                        v1 = self.alpha * Q_values[t,second_best_option-1]/np.sqrt(Q_values_var[t,second_best_option-1]) + self.beta*np.sqrt(Q_values_var[t,second_best_option-1])
                        v2 = self.alpha * Q_values[t,worst_option-1]/np.sqrt(Q_values_var[t,worst_option-1]) + self.beta*np.sqrt(Q_values_var[t,worst_option-1])
                
                    if self.drift_rate_model == "q_mean_unc_bonus":
                        v0 = self.alpha * Q_values[t,best_option-1] + self.beta*np.sqrt(Q_values_var[t,best_option-1])
                        v1 = self.alpha * Q_values[t,second_best_option-1] + self.beta*np.sqrt(Q_values_var[t,second_best_option-1])
                        v2 = self.alpha * Q_values[t,worst_option-1] + self.beta*np.sqrt(Q_values_var[t,worst_option-1])
                    ...
                    if self.eam_model == "race":
                        theta_eam = {'v0': v0, 'v1': v1, 'v2': v2,'a': self.a,'t': self.ndt}
